...
lazy platform => method() {
		...
		if(  $^O eq 'darwin' && which('brew') ) {
			$system = Orbital::Payload::Sys::System::MacOSHomebrew->new( @opt );
		} elsif( $^O eq 'MSWin32' ) {
			$system = Orbital::Payload::Sys::System::MSYS2->new( @opt );
		} else {
			$system = Orbital::Payload::Sys::System::Debian->new( @opt );
		}
};
